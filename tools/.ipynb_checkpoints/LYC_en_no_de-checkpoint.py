...
class NestFuse_light2_nodense(nn.Module):
    def __init__(self, nb_filter, input_nc=[3,2], output_nc=1, deepsupervision=False):
        super(NestFuse_light2_nodense, self).__init__()
        self.YG_channel = input_nc[0]
        self.DEM_channel = input_nc[1]
        ...
        # encoder
        self.conv_rgb = ConvLayer(self.YG_channel, output_filter, 1, stride)
        self.conv_dem = ConvLayer(self.DEM_channel, output_filter, 1, stride)
    ...
    def encoder(self, input):
        # print(input.shape)
        if input.shape[1] == self.YG_channel:
    ...
    def decoder(self, f_en):
        ...
        # print(x1_3.shape)
        if isinstance(f_en, list):
            output1 = self.gap(f_en[3])
        else:
            output1 = self.gap(f_en)
