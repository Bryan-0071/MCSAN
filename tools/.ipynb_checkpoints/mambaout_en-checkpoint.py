...
class UpsampleLayer(nn.Module):
    def __init__(self, in_channels=198, out_channels=96, norm_layer=partial(nn.LayerNorm, eps=1e-6)):
        super().__init__()
...
class DownsampleLayer(nn.Module):
    r""" Code modified from InternImage:
        https://github.com/OpenGVLab/InternImage
    """
    def __init__(self, in_channels=96, out_channels=198, norm_layer=partial(nn.LayerNorm, eps=1e-6), stride=2):
        super().__init__()
...
class GatedCNNBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # g torch.Size([4, 16, 128, 341])
        # i torch.Size([4, 16, 128, 213])
        # c torch.Size([4, 16, 128, 128])

        c = c.permute(0, 3, 1, 2) # [B, H, W, C] -> [B, C, H, W]
        # torch.Size([4, 128, 16, 128])
