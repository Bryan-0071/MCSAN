...
class Fusion_network_Transformer(nn.Module):
    def __init__(self, nC, input_resolution=[128, 64, 32, 16], depth=[2, 2, 2, 2], num_heads=[8, 8, 8, 8], expand_size=[0, 0, 0, 0], focal_window=[37, 19, 9, 5]):
    ...
    def forward(self, en_ir):
        ...
        if len_encoder != 4:
            f1_0 = self.fusion_block1(en_ir)
            return f1_0
        else:
            f1_0 = self.fusion_block1(en_ir[0])
            f2_0 = self.fusion_block2(en_ir[1])
            f3_0 = self.fusion_block3(en_ir[2])
            f4_0 = self.fusion_block4(en_ir[3])
