...
class Transblock(nn.Module):
    def __init__(self,dim, input_resolution, depth, num_heads, expand_size, window_size=7, expand_layer="all",
        ...
        self.conv1 = ConvLayer(dim * 2, dim, 1, 1)
        self.conv1_single = ConvLayer(dim, dim, 1, 1)
        
        self.conv2 = ConvLayer(dim, dim, 1, 1)
        # self.bn1 = nn.BatchNorm2d
    def forward(self, en_ir,en_vi):
        # print("en_ir = ", en_ir.shape)
        # print("en_vi = ", en_vi.shape)
        if en_vi is None:
            # print("------------------")
            x = self.conv1_single(en_ir)
        else:
            x = torch.cat((en_ir, en_vi), 1)
            x = self.conv1(x)
        # x = x.view(x.shape[0], x.shape[1], self.input_resolution[0] * self.input_resolution[1]).permute(0, 2, 1).contiguous()
        x = x.view(x.shape[0], x.shape[1], self.input_resolution[0] * self.input_resolution[1]).permute(0, 2, 1).contiguous()
        
        for blk in self.transblock:
...
class Fusion_network_Transformer(nn.Module):
    def __init__(self, nC, input_resolution=[256, 128, 64, 32], depth=[2, 2, 2, 2], num_heads=[8, 8, 8, 8], expand_size=[0, 0, 0, 0], focal_window=[37, 19, 9, 5]):
        super(Fusion_network_Transformer, self).__init__()
        '''
        self.fusion_block1 = Transblock(dim=nC[0], input_resolution=(input_resolution[0], input_resolution[0]),
                                        depth=depth[0], num_heads=num_heads[0], expand_size=expand_size[0], focal_window=focal_window[0])
        self.fusion_block2 = Transblock(dim=nC[1], input_resolution=(input_resolution[1], input_resolution[1]),
                                        depth=depth[1], num_heads=num_heads[1], expand_size=expand_size[1], focal_window=focal_window[1])
        self.fusion_block3 = Transblock(dim=nC[2], input_resolution=(input_resolution[2], input_resolution[2]),
                                        depth=depth[2], num_heads=num_heads[2], expand_size=expand_size[2], focal_window=focal_window[2])
        self.fusion_block4 = Transblock(dim=nC[3], input_resolution=(input_resolution[3], input_resolution[3]),
                                        depth=depth[3], num_heads=num_heads[3], expand_size=expand_size[3], focal_window=focal_window[3])
        '''
        params = zip(nC, input_resolution, depth, num_heads, expand_size, focal_window)
        for i, (dim, input_res, depth, num_heads, expand_size, focal_window) in enumerate(params, start=1):
            # print(i)
            setattr(self, f'fusion_block{i}', Transblock(dim=dim, input_resolution=(input_res, input_res),
                                                        depth=depth, num_heads=num_heads,
                                                        expand_size=expand_size, focal_window=focal_window))
        
    def forward(self, en_ir, en_vi):
        # f1_0 = self.fusion_block1(en_ir[0], en_vi[0])
        len_encoder = len(en_ir)
        if len_encoder == 1:
            f1_0 = self.fusion_block1(en_ir[0], en_vi[0])
            return f1_0
        elif len_encoder == 2:
...
class Fusion_network_hi(nn.Module):
    def __init__(self, nC, input_resolution=256, depth=2, num_heads=4, expand_size=0, focal_window=9):
        super(Fusion_network_hi, self).__init__()
        
        self.fusion_block1 = Transblock(dim=nC, input_resolution=(input_resolution, input_resolution),
                                        depth=depth, num_heads=num_heads, expand_size=expand_size, focal_window=focal_window)
