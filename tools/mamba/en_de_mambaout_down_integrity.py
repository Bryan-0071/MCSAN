...
class NestFuse_light2_nodense(nn.Module):
    def __init__(self, nb_filter, input_nc=1, output_nc=1, deepsupervision=False):
        ...
        self.DB3_1 = nn.Sequential(DownsampleLayer(nb_filter[1] + nb_filter[2], nb_filter[2]),
                                   self._make_layer(dims[2], num_layers[2]))
        self.DB3_2 = nn.Sequential(DownsampleLayer(nb_filter[2] * 2 + nb_filter[1], nb_filter[2], stride=1),
                                   self._make_layer(dims[2], num_layers[2]))

        self.DB4_1 = nn.Sequential(DownsampleLayer(nb_filter[2]+ nb_filter[3], nb_filter[3]),
                                   self._make_layer(dims[3], num_layers[3]))
        self.DB4_2 = nn.Sequential(DownsampleLayer(nb_filter[2]+ nb_filter[3] * 2, nb_filter[3], stride=1),
                                   self._make_layer(dims[3], num_layers[3]))
        ...
        self.fc = nn.Linear(nb_filter[3], 1)
    ...
    def forward(self, input):
        ...
        x1_dem = self.EB1_0(x_dem)
        # ([2, 64, 128, 128])

        x2_dem = self.EB2_0(x1_dem)
        # ([2, 112, 64, 64])

        x3_dem = self.EB3_0(x2_dem)
        # ([2, 160, 32, 32])

        x4_dem = self.EB4_0(x3_dem)
        # ([2, 208, 16, 16])
