...
from tools.mambaout_en import GatedCNNBlock, DownsampleLayer, UpsampleLayer, StemLayer, MlpHead
...
class NestFuse_light2_nodense(nn.Module):
    ...
    def __init__(self, nb_filter, input_nc=1, output_nc=1, deepsupervision=False):
        ...
        self.norm1 = DownsampleLayer(dims[0], dims[1])
        self.norm2 = DownsampleLayer(dims[1], dims[2])
        self.norm3 = DownsampleLayer(dims[2], dims[3])

        self.norm1_dsm = DownsampleLayer(dims[0], dims[1])
        self.norm2_dsm = DownsampleLayer(dims[1], dims[2])
        self.norm3_dsm = DownsampleLayer(dims[2], dims[3])
        ...
        '''
        self.down1 = DownsampleLayer(dims[0], dims[0])
        ...
        self.head = MlpHead(dims[3],num_classes=1)
    ...
    def _make_layer(self, channels, num_layers):
        ...
        for _ in range(num_layers):
            layers.append(GatedCNNBlock(channels, drop_path=0))
    ...
    def forward(self, input):
        ...
        x1_0 = self.EB1_0(x)
        x1_0 = self.norm1(x1_0)
        # ([2, 64, 128, 128])

        x2_0 = self.EB2_0(x1_0)
        x2_0 = self.norm2(x2_0)
        # ([2, 112, 64, 64])

        x3_0 = self.EB3_0(x2_0)
        x3_0 = self.norm3(x3_0)
        # ([2, 160, 32, 32])
        ...
        x1_dem = self.norm1(x1_dem)
        # ([2, 64, 128, 128])

        x2_dem = self.EB2_0(x1_dem)
        x2_dem = self.norm1(x2_dem)
        # ([2, 112, 64, 64])

        x3_dem = self.EB3_0(x2_dem)
        x3_dem = self.norm1(x3_dem)
        # ([2, 160, 32, 32])
