...
class NestFuse_light2_nodense(nn.Module):
    def __init__(self, nb_filter, input_nc=1, output_nc=1, deepsupervision=False):
        ...
        self.conv_dem = StemLayer(in_channels=5, out_channels=dims[0])
        

        # set_trace()
        self.EB1_0 = nn.Sequential(GatedCNNBlock(dims[0]))

        self.EB2_0 = nn.Sequential(DownsampleLayer(dims[0], dims[1]),
                                   GatedCNNBlock(dims[1]))

        self.EB3_0 = nn.Sequential(DownsampleLayer(dims[1], dims[2]),
                                   GatedCNNBlock(dims[2]))
        
        self.EB4_0 = nn.Sequential(DownsampleLayer(dims[2], dims[3]),
                                   GatedCNNBlock(dims[3]))

        # decoder
        # self.DB1_1 = block(nb_filter[0] + nb_filter[1], nb_filter[0], kernel_size, 1)
        self.DB2_1 = nn.Sequential(DownsampleLayer(nb_filter[0] + nb_filter[1], nb_filter[1]),
                                   GatedCNNBlock(dims[1]))
        ...
        self.DB3_1 = nn.Sequential(DownsampleLayer(nb_filter[1] + nb_filter[2], nb_filter[2]),
                                   GatedCNNBlock(dims[2]))
        self.DB3_2 = nn.Sequential(DownsampleLayer(nb_filter[2] * 2 + nb_filter[1], nb_filter[2]),
                                   GatedCNNBlock(dims[2]))

        self.DB4_1 = nn.Sequential(DownsampleLayer(nb_filter[2]+ nb_filter[3], nb_filter[3]),
                                   GatedCNNBlock(dims[3]))
        self.DB4_2 = nn.Sequential(DownsampleLayer(nb_filter[2]+ nb_filter[3] * 2, nb_filter[3]),
                                   GatedCNNBlock(dims[3]))
        self.DB4_3 = nn.Sequential(DownsampleLayer(nb_filter[2]+ nb_filter[3] * 3, nb_filter[3]),
                                   GatedCNNBlock(dims[3]))
        ...
        self.fc = nn.Linear(nb_filter[0], 3)
    ...
    def decoder(self, f_en):
        ...
        x1_1 = self.DB2_1(torch.cat([self.pool(f_en[0]), f_en[1]], 1).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        x2_1 = self.DB3_1(torch.cat([self.pool(f_en[1]), f_en[2]], 1).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        set_trace()
        x2_2 = self.DB3_2(torch.cat([self.pool(f_en[2]), self.pool(x1_1), x2_1], 1).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)

        x3_1 = self.DB4_1(torch.cat([f_en[3], self.pool(f_en[2])], 1).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        x3_2 = self.DB4_2(torch.cat([self.pool(f_en[3]), x3_1, self.pool(x2_1)], 1).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        x3_3 = self.DB4_3(torch.cat([self.pool(f_en[3]), x3_1, x3_2, self.pool(x2_2)], 1).permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        # set_trace()
