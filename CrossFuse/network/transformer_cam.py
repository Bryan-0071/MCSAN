...
class cross_encoder(nn.Module):
    def __init__(self, img_size, patch_size, embed_dim, num_patches, depth_self, depth_cross, n_heads=8,
                 mlp_ratio=4., qkv_bias=True, p=0., attn_p=0.):
        super().__init__()
        self.num_patches = num_patches
    ...
    def forward(self, x1, x2, shift_flag=True):
        ...
        if shift_flag:
            ...
            shifted_x1 = torch.roll(x1_atten, shifts=(-shift_num1, -shift_num1), dims=(2, 3))
            shifted_x2 = torch.roll(x2_atten, shifts=(-shift_num1, -shift_num1), dims=(2, 3))

            x1_atten, x2_atten, _ = self.self_atten_block2(shifted_x1, shifted_x2)
            # roll 2
            shift_num2 = x1_atten.shape[2] // 2
            ...
            roll_x_self1 = torch.roll(x1_atten, shifts=(shift_num2, shift_num2), dims=(2, 3))
            roll_x_self2 = torch.roll(x2_atten, shifts=(shift_num2, shift_num2), dims=(2, 3))
        else:
            # 第二个self-attention
        ...
        # return out, x_self1, x_self2
        return x_cross1

class cross_encoder_test(nn.Module):
